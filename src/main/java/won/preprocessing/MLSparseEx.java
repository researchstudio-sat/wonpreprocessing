...
/**
 * Code copy of class {@link com.jmatio.types.MLSparse} to change the following things:
 * - resize matrix
 * - get current number of non zero entries
 * - extends from MLSparse cause MatFileWriter expects type {@link com.jmatio.types.MLSparse}
 */
public class MLSparseEx extends MLSparse
{
  int nzmax;
  ...
  // added in comparison to {@link com.jmatio.types.MLSparse}
   ...
   * @param nzmax
   */
  public MLSparseEx(String name, int[] dims, int attributes, int nzmax)
  {
  ...
  /**
   * resize matrix to new dimensions and nzmax value.
   * function added in comparison to {@link com.jmatio.types.MLSparse}
   *
   * @param dims new dimensions
   * @param nzmax new nzmax value
   * @return resized matrix
   */
  public MLSparseEx resize(int dims[], int nzmax) {

    MLSparseEx newMatrix = new MLSparseEx(getName(), dims, attributes, nzmax);
    for (IndexMN index : indexSet) {
      if (index.m < dims[0] && index.n < dims[1]) {
        newMatrix.set(get(index.m, index.n), index.m, index.n);
      }
    }
    return newMatrix;
  }
  ...
  /**
   * function added in comparison to {@link com.jmatio.types.MLSparse}
   * @return number of current nonzero entries
   */
  public int getCurrentNZ()
  {
